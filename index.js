...
async function fetchVideoTranscript(videoId) {
  try {
    ...
    // First try to get the transcript directly without language specification
    try {
      console.log('Trying direct fetch without language...');
    ...
    // If direct fetch fails, try with specific configurations
    const configs = [
      { lang: 'en' },
      { lang: 'ms' },
    ...
    for (const config of configs) {
      try {
        ...
        const transcript = await YoutubeTranscript.fetchTranscript(videoId, {
          ...config,
          client: 'web' // Try with web client
        });
    ...
    // If all attempts fail, try one last time with raw API call
    try {
      console.log('Attempting raw API call...');
      const response = await fetch(`https://www.youtube.com/watch?v=${videoId}`);
      const html = await response.text();
      
      // Check if captions exist in the page
      if (html.includes('"captions":') || html.includes('"playerCaptionsTracklistRenderer"')) {
        console.log('Captions detected in page, but unable to fetch. This might be a technical limitation.');
        throw new Error('Captions exist but cannot be accessed. This might be due to technical limitations.');
      }
    } catch (error) {
      console.log('Raw API check failed:', error.message);
    }

    throw new Error('No accessible captions found. The video might have restricted captions or they might be in an unsupported format.');
  } catch (error) {
