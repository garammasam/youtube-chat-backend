...
async function fetchVideoTranscript(videoId) {
  try {
    ...
    for (const config of configs) {
      try {
        console.log('Attempting fetch with config:', config);
    ...
    if (!transcript) {
      // Try one last time with a direct page check
      try {
        console.log('Performing direct page check...');
        const response = await fetch(`https://www.youtube.com/watch?v=${videoId}`);
        const html = await response.text();
        
        if (html.includes('"captions":') || html.includes('"playerCaptionsTracklistRenderer"')) {
          throw new Error('Captions exist but could not be accessed. This might be due to YouTube API restrictions.');
        } else {
          throw new Error('No captions found for this video.');
        }
      } catch (error) {
        console.log('Direct page check failed:', error.message);
        throw error;
      }
    }
    ...
    throw lastError || new Error('Unable to fetch captions. Please try another video.');
  } catch (error) {
